...
Reads are assembled with PEAR; extraction uses the assembled FASTQ. The lineage insert cut from the assembled read is **reverse-complemented** before it is stored as `LB`, so it matches the reference orientation used by downstream annotation (`LB` / `LR` / `query` stay consistent through denoise).
...
- **R1:** UMI at the 5′ end  
- **R2:** primer5 — lineage insert — primer3 (primers matched as forward sequences from the `darlin-core` locus config)
...
| `<output-dir>/<sample-id>/reads_<...>/denoised_barcodes.tsv` | Barcode-level summary with `LR` (lineage barcode), `UMIs`, and `query` (same sequence as `LR`, for merging with `annotated.tsv`). |
| `<output-dir>/<sample-id>/reads_<...>/annotated.tsv` | Allele annotation output generated by `darlin-core` (written during `annotate`). |
| `<output-dir>/<sample-id>/reads_<...>/alleles_by_umis.tsv` | Final clone-level table for one parameter combination (written during `annotate` / finalize merge): columns `LR`, `UMIs`, `mutations`, `aligned_query`, `aligned_ref` (left merge from `annotated.tsv`; rows without a match keep empty annotation fields). |
