...
def step_finalize(
    *,
    ...
    merged_rows = len(final)
    rows_with_aligned_pair = 0
    if "aligned_query" in final.columns and "aligned_ref" in final.columns:
        rows_with_aligned_pair = int(
            (final["aligned_query"].notna() & final["aligned_ref"].notna()).sum()
        )
    ...
    logger.info(
        "Finalize: merged_rows=%s rows_with_aligned_query_ref=%s output_rows=%s -> %s",
        merged_rows,
        rows_with_aligned_pair,
        len(out),
        combo.alleles_tsv,
    )
