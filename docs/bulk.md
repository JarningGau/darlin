...
| Argument | Meaning |
|----------|---------|
| `--sample-id` | Sample identifier used for naming the output directory and log file. |
...
| `<output-dir>/<sample-id>/reads_<...>/<sample-id>_alleles.csv` | Final clone-level output for one parameter combination. |
...
darlin bulk finalize \
  --sample-id L141_CA \
  --output-dir ./output \
  --denoised-barcodes ./output/L141_CA/reads_1_u_1_l_0.01/denoised_barcodes_with_query.tsv \
