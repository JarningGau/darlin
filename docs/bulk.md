...
| `<output-dir>/<sample-id>/reads_<...>/alleles_by_umis.tsv` | Final clone-level table for one parameter combination (written during `annotate` / finalize merge): columns `LR`, `UMIs`, `mutation`, `aligned_query`, `aligned_ref` (left merge from `annotated.tsv`; rows without a match keep empty annotation fields). |
